...
def validate(c):
    ...
    for i, s in enumerate(c["series"]):
        ...
        if cat_chart:
            ...
            wide = 3 if c["type"] in ("scatter", "line") else 2
    ...
    if len(c["series"]) > 6 and not c.get("highlight"):
        warnings.append("%d series: past ~6 lines a reader cannot track them; "
                        "set `highlight` (the rest become grey context with a "
                        "right-hand label column) or use small multiples"
                        % len(c["series"]))
    for w in warnings:
